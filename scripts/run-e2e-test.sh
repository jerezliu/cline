...
usage() {
    echo "Usage: $0 --workspace <path> --task <description> --api-key <key> [--script-path <path>] [--wait-seconds <seconds>]"
    exit 1
}
...
"$PLAN_ACT_SCRIPT_PATH" --task "$TASK" --api-key "$API_KEY" --workspace "$WORKSPACE"
SCRIPT_EXIT_CODE=$?
