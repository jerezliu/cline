...
# This file is automatically generated by the Cline agent.
# Do not edit this file manually.

SYSTEM_PROMPT_CLAUDE4 = """You are Cline, a highly skilled software engineer with extensive knowledge in many programming languages, frameworks, design patterns, and best practices.
...
- You can use search_files to perform a regex search across files in a specified directory, outputting context-rich results that include surrounding lines. This is particularly useful for understanding code patterns, finding specific implementations, or identifying areas that need refactoring.
...
def get_browser_action_prompt(supports_browser_use: bool, browser_settings: dict) -> str:
    if not supports_browser_use:
        return ""
    return f"""
## browser_action
...
- The browser window has a resolution of **{browser_settings['viewport']['width']}x{browser_settings['viewport']['height']}** pixels. When performing any click actions, ensure the coordinates are within this resolution range.
...
- coordinate: (optional) The X and Y coordinates for the \`click\` action. Coordinates should be within the **{browser_settings['viewport']['width']}x{browser_settings['viewport']['height']}** resolution.
...
def get_user_instructions(
    global_cline_rules_file_instructions: str | None = None,
    local_cline_rules_file_instructions: str | None = None,
    local_cursor_rules_file_instructions: str | None = None,
    local_cursor_rules_dir_instructions: str | None = None,
    local_windsurf_rules_file_instructions: str | None = None,
    cline_ignore_instructions: str | None = None,
    preferred_language_instructions: str | None = None,
) -> str:
    custom_instructions = ""
    if preferred_language_instructions:
        custom_instructions += preferred_language_instructions + "\\n\\n"
    if global_cline_rules_file_instructions:
        custom_instructions += global_cline_rules_file_instructions + "\\n\\n"
    if local_cline_rules_file_instructions:
        custom_instructions += local_cline_rules_file_instructions + "\\n\\n"
    if local_cursor_rules_file_instructions:
        custom_instructions += local_cursor_rules_file_instructions + "\\n\\n"
    if local_cursor_rules_dir_instructions:
        custom_instructions += local_cursor_rules_dir_instructions + "\\n\\n"
    if local_windsurf_rules_file_instructions:
        custom_instructions += local_windsurf_rules_file_instructions + "\\n\\n"
    if cline_ignore_instructions:
        custom_instructions += cline_ignore_instructions
